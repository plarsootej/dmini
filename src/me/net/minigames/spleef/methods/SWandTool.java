...
public class SWandTool {
	...
	private static int wandtoolid = ConfigHandler.MainSpleef.getInt("Wand-Tool-ID");
	...
	public static void WandTool(PlayerInteractEvent event) {
		...
		if(inhand.getType() == Material.getMaterial(wandtoolid)) {
			if(event.getAction() == Action.LEFT_CLICK_BLOCK) {
				...
				if(loc2.isEmpty()) {
					player.sendMessage("§6[DS] §cPos #1 Selected §fRight click: Select pos #2");
				}else {
					player.sendMessage("§6[DS] §cPos #1 and #2 Selected!");
				}
				...
				if(loc1.isEmpty()) {
					player.sendMessage("§6[DS] §cPos #2 Selected §fLeft cick: Select pos #1");
				}else {
					player.sendMessage("§6[DS] §cPos #1 and #2 Selected! Type §f/spleef setup §c For more information");
				}
