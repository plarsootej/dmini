...
import me.net.minigames.spleef.methods.SCreateLose;
...
public class SCommandCreateLose {
	
	@SuppressWarnings("deprecation")
	public static void SetLoseLevel(CommandSender sender, String[] args) {
		if(sender instanceof Player) {
			if(SpleefMain.SpleefMain.pdata.arenanames.containsKey(args[1])) {
				if(args.length == 2 && args[0].equalsIgnoreCase("setloselevel")) {
					...
					if (!(SWandTool.loc1.isEmpty()) && !(SWandTool.loc2.isEmpty())) {
						...
						SCreateLose.CreateLose(arenaname);
						...
						sender.sendMessage("§aLose level Succesfully created for arena §f " + arenaname);
					...
					sender.sendMessage("§cCommand Ussage: §f/spleef setloselevel <arenaname>");
				}
			}else {
